...
def coinMarketCapRank(numCoins):
    ...
    #       because rank might change during the iterations!
    ...
    for i in range(0,int(numCoins/100)):
        ...
        #print("START: " + str(((100*i)+1)))
    if numCoins % 100 > 0:
        #print(len(coin_ranked))
        #print("LAST_START: " + str((((100*(i+1))+1))))
        #print("LAST_UNTIL: " + str(numCoins-len(coin_ranked)))
        coin_ranked.update(coinmarketcap.ticker(start=((100*(i+1))+1), limit=(numCoins-len(coin_ranked)))['data'])
        #if (len(coin_ranked) == numCoins):
    return coin_ranked
...
coins_ranked = coinMarketCapRank(numCoins)

coins = []
for c in coins_ranked.values():
    coins.append(Coin(c))
    logger.info('Start searcher for ' + c['name'])
